...
@Component({
  ...
  styleUrls: ['./cerradas.component.css']
})
export class CerradasComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe({
      next: (params) => {
        ...
        this.VerId=id
        if (id) {
          this.dBConectionService.  getSolicitudSuperAreaCerradas(id)
  ...
  migajaPanMenu() {
    this.items = [
      ...
      { label: 'Pendientes Por Firmar', routerLink:'/supervisor_firma/' },
      { label: 'Cerradas' }
    ];
  }
  ...
  menuItems() {
    this.itemsMenu = [{
      ...
      },
      {
        ...
        icon: 'fas fa-check',
        routerLink:(['/cerradas_Trabajo-temporal/']),
        command: () => {
